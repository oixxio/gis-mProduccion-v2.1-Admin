(function () { 
    ...
    controller('devProjectsController', ['$scope','$location','$http','$parse', '$window','devAppInfoFactory','$timeout','loginFactory','$uibModal','$log','$rootScope',
    			function($scope,$location,$http,$parse,$window,devAppInfoFactory,$timeout,loginFactory,$uibModal,$log,$rootScope){

    	var databaseRef = firebase.database();
    	var storageRef = firebase.storage().ref();

        $scope.projectsInfoDev = [];
        $scope.State = ""
        ...
        $scope.readDevApps = function() {
            var user = firebase.auth().currentUser;
                ...
                for (var i = 0; i < Object.keys($scope.appsOnline).length; i++) {
                    ...
                    if (auxProjects.Developer === user.uid) {
                        ...
                        $scope.projectsInfoDev.push({   'projectAppName' : project.AppBase,
